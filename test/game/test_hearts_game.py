...
class TestHeartsGame(unittest.TestCase):
    def test_play_round(self):
        ...
        game.play_round()
        # assert
        self.assertEqual(26, np.sum(game.round.scores),
                         'Sum of all points should be equal to 26')
        self.assertEqual([0] * 4, [len(hand) for hand in game.round.hands],
                         'All players should have empty hands')
        self.assertEqual([13] * 4, [len(cards) for cards in game.round.played_cards],
                         'All players should have played 13 cards')
        self.assertEqual(
            52,
            np.sum([len(cards_for_player) for cards_for_player in game.round.taken_cards]),
    ...
    def test_passing_cards_changes(self):
        game = get_game_with_random_players()
        self.assertEqual(PassDirection.LEFT, game.round.pass_direction)

        game.next_round()
        self.assertEqual(PassDirection.RIGHT, game.round.pass_direction)

        game.next_round()
        self.assertEqual(PassDirection.ACROSS, game.round.pass_direction)

        game.next_round()
        self.assertEqual(PassDirection.NO_PASSING, game.round.pass_direction)
